...
import type { ContentSelectType } from "@/hooks/useSelectContent";
...
interface ContentItemProps {
  ...
  propsSelectContent: ContentSelectType
}

export default function ContentItem({ item, displayOptions, propsSelectContent, onScrollToElement }: ContentItemProps) {
  ...
  const handleSelectElement = (itemId: string) => {
    if (itemId) {
      propsSelectContent.selectContent(itemId);
      onScrollToElement(itemId);
    }
  }
  ...
  return (
    <div style={indentStyle} className="flex">
      ...
      >
        <span className={clsx("text-gray-700 break-words group-hover:opacity-50", { 'opacity-50': propsSelectContent.id === item.id })}>
